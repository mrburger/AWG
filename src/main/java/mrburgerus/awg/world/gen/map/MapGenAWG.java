...
/**
 * Created by mrburgerUS on 4/30/2017.
 */
public class MapGenAWG
{
    protected int range;
    ...
    public void generate(World world, int i, int j, ChunkPrimer chunkPrimer)
    {
        int k = range;
        worldObj = world;
        ...
        for(int i1 = i - k; i1 <= i + k; i1++)
        {
            for(int j1 = j - k; j1 <= j + k; j1++)
            {
                ...
                recursiveGenerate(worldObj, i1, j1, i, j, chunkPrimer);
            }
        }
    }
    ...
    protected void recursiveGenerate(World world, int i, int j, int k, int l, ChunkPrimer primer)
    {
    }
